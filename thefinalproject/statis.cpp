...
int Statis::Stat_ROOM_NUMBER()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from ROOMS where  ROOM_NUMBER BETWEEN '0' AND 4000'") ;
    while(requete.next())
    {
            count++ ;
    }
...
void Statis::paintEvent(QPaintEvent *)
{
    ...
    painter.setBrush(Qt::white);
        ...
        ui->label_1->setText("TYPE  between A-Z") ;
        painter.setBrush(Qt::cyan);
        painter.drawPie(size,16*x,16*y);
        ui->label_2->setText("TYPE between a-z") ;
        painter.setBrush(Qt::blue);
        painter.drawPie(size,16*(y+x),16*z);
        ui->label_3->setText("number between 0-4000") ;
